...
class Action:
    ...
    def __init__(self, value: Union[tuple, torch.Tensor]) -> None:
        """
        Overview:
            Initializes the Action with the given value.
        Arguments:
            - value (:obj:`tuple`): The value of the action. Can be either an integer or a numpy array.
        """
        if isinstance(value, torch.Tensor):
            self.value = tuple(value.tolist())
        else:
...
class Node(object):
    ...
    @property
    def children(self) -> dict[Action, "Node"]:
        """
        Overview:
...
class MCTS(object):
    ...
    def __init__(self, cfg: EasyDict, simulate_env: AlphaZXEnv) -> None:
        """
        Overview:
    ...
    def get_next_action(
            self,
            policy_value_func: Callable[[Any], tuple[list[list], float]],
        ...
        for action, action_node in self.root.children.items():
            action_visits.append((action.value, action_node.visit_count))
        ...
        # TODO: Should actions that were never visited during MCTS be left out?
        # self.root_sampled_actions = actions[np.nonzero(action_probs)[0]]
        self.root_sampled_actions = actions
    ...
    def _simulate(self, node: Node, simulate_env: AlphaZXEnv,
                  policy_value_func: Callable[[Any], tuple[list[list], float]]) -> None:
        """
        Overview:
        ...
        while not node.is_leaf():
            ...
            if action is None:
                break
            if not isinstance(action.value, tuple):
                raise Exception(f'Expected action value {action.value} to be a tuple.')
            simulate_env.step(action.value)

        done, done_reward = simulate_env.get_done_reward()
    ...
    def _expand_leaf_node(self, node: Node, simulate_env: AlphaZXEnv,
                          policy_value_func: Callable[[Any], tuple[list[list], float]]) -> float:
        """
        Overview:
        ...
        # Call the policy_value_func function to compute the policy parameters and state value, and return a
        # tuple of sampled actions and the value of the leaf node.
        sampled_actions, leaf_value = policy_value_func(simulate_env)

        node.legal_actions = []

        uniques, counts = torch.unique(torch.tensor(sampled_actions), dim=0, return_counts=True, sorted=True)
        sampled_actions = uniques.tolist()
        empirical_probs = counts / counts.sum()

        self.num_of_sampled_actions = len(sampled_actions)

        for i in range(self.num_of_sampled_actions):
            node.children[Action(sampled_actions[i])] = Node(parent=node, prior_p=empirical_probs[i])
            node.legal_actions.append(Action(sampled_actions[i]))
    ...
    def get_sampled_actions(self) -> list[tuple]:
        """
        Overview:
