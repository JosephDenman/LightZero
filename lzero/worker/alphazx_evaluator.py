...
@SERIAL_EVALUATOR_REGISTRY.register('alphazx')
class AlphaZXEvaluator(ISerialEvaluator):
    """
    Overview:
        AlphaZX Evaluator class which handles the evaluation of the trained policy.
    Interfaces:
        ...
        if get_rank() == 0:
            ...
            with self._timer:
                while not eval_monitor.is_finished():
                    ...
                    time_steps = to_tensor(time_steps, dtype=torch.float32)
                    for env_id, t in time_steps.items():
