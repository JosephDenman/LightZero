...
from ding.utils import SERIAL_COLLECTOR_REGISTRY, EasyTimer
from ding.utils import build_logger, get_rank, get_world_size, allreduce_data
from ding.worker import ISerialCollector
from ding.worker.collector.base_serial_collector import CachePool, TrajBuffer, INF, to_tensor_transitions


@SERIAL_COLLECTOR_REGISTRY.register('episode_alphazx')
class AlphaZXCollector(ISerialCollector):
    """
        Overview:
            AlphaZX collector for collecting episodes of experience during self-play or playing against an opponent.
            This collector is specifically designed for the AlphaZX algorithm.
        Interfaces:
    ...
    def __init__(
        ...
        """
        Overview:
            Initialize the AlphaZX collector with the provided environment, policy, and configurations.
        Arguments:
    ...
    def collect(self,
        ...
        while True:
            ...
            interaction_duration = self._timer.value / len(time_steps)
            for env_id, timestep in time_steps.items():
                with self._timer:
